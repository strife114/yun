...
2. 安装kubeeasy
   ...
   kubeeasy install depend --host 192.168.100.10,192.168.100.20 --user root --password 000000 --offline-file /opt/dependencies/base-rpms.tar.gz 
...
3. 配置SSH免密
   ...
   ```
   # 完成集群节点的连通性检测
   kubeeasy check ssh --host 192.168.100.10,192.168.100.20 --user root --password 000000
   
   # 集群所有节点间的免密钥配置
   kubeeasy create ssh-keygen --master 1192.168.100.10 --worker 192.168.100.20 --user root --password 000000
   ```
